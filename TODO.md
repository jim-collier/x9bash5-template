...
| 20250716 |        | 5 |  3   |          |    |           | Finish `fFormatNum()`
| 20250716 |        | 5 |  2   |          |    |           | Update `fTimer_GetET()` to use fFormatNum().
| 20250716 |        | 5 |  3   |          |    |           | Add `fUnitTest_TimeFunction()` (all in-line). | Args <avgETVar> [stdDevVar] [loopIterations] [runCount]
| 20250711 |        | 4 |  2   |          |    |           | Write basic `fArrayFromStr()` | Std safe syntax is convoluted just enough, and done frequently enough, to justify. Use *nameref* vars.
| 20250711 |        | 4 |  2   |          |    |           | Write basic `fArrayFromFile()`
| 20250711 |        | 4 |  2   |          |    |           | Write basic `fArrayToStr()`
...
| 20250715 |        | 5 |  4   |          |    |           | Provide Bash-style default arguments for function arguments, and refactor custom defaults if appropriate.
...
| 20250715 |        | 2 |  2   |          |    |           | Add functions that 'safe-escape' and de-safe-escape' strings. (de-`sed`ed)
